...
<div class="col-md-12  col-lg-5 align-self-center text-right p-b-9 {{ $page['list_page_actions_size'] ?? '' }} {{ $page['list_page_container_class'] ?? '' }}"
    ...
    <div id="list-page-actions">
        ...
        @php
            $client_email_user = \App\Models\User::where('clientid', $bill->bill_clientid)->where('account_owner', 'yes')->first();
            $client_email_address = $client_email_user ? $client_email_user->email : 'client';
        @endphp
        <button type="button" data-toggle="tooltip" title="{{ cleanLang(__('lang.send_email')) }}"
            ...
            data-confirm-text="Send invoice to: &lt;span style='color:#e91e63;'&gt;{{ $client_email_address }}&lt;/span&gt;"
            data-url="{{ urlResource('/invoices/'.$bill->bill_invoiceid.'/resend') }}"
            id="invoice-action-email-invoice"><i class="ti-email"></i></button>
        @endif

        <!--overdue invoice reminder-->
        @if(auth()->user()->role->role_invoices > 1 && $bill->bill_status == 3)
        <button type="button" data-toggle="tooltip" title="{{ cleanLang(__('lang.send_overdue_reminder')) }}"
            ...
            id="invoice-action-email-invoice"><i
                class="mdi mdi-email-alert display-inline-block m-t--4 font-32"></i></button>
        @endif
        ...
        <span class="dropdown">
            ...
            <div class="dropdown-menu" aria-labelledby="listTableAction">
                ...
                <a class="dropdown-item actions-modal-button js-ajax-ux-request reset-target-modal-form"
                    ...
                    data-url="{{ url('/invoices/'.$bill->bill_invoiceid.'/change-status') }}"
                    data-action-url="{{ url('invoices/'.$bill->bill_invoiceid.'/change-status') }}"
                    data-action-method="POST" data-loading-target="actionsModalBody"
                    data-modal-title="@lang('lang.change_invoice_status')">
                    <i class="sl-icon-settings display-inline-block p-r-5"></i>@lang('lang.change_invoice_status')</a>
                <!--edit-->
                ...
                <!--pritable view-->
                <a class="dropdown-item" href="{{ url('/invoices/'.$bill->bill_invoiceid.'/pdf?view=preview') }}" target="_blank"><i
                        class="ti-printer"></i>{{ cleanLang(__('lang.printable_view')) }}</a>
            </div>
