...
function middlewareBootSettings() {
    ...
    //invoice custom statuses
    $settings['invoice_statuses'] = [];
    $settings['invoice_custom_statuses'] = [];
    foreach (\App\Models\InvoiceStatus::get() as $status) {
        $id = $status->invoicestatus_id;
        $color = $status->invoicestatus_color;
        $title = $status->invoicestatus_title;
        $settings['invoice_statuses'] += [
            $id => $color,
        ];
        $settings['invoice_custom_statuses'] += [
