...
<div class="mb-3">
    ...
    <input
        type="text"
        id="{{ $displayId }}"
        class="form-control sd_datepicker datepicker-range-{{ $componentId }} {{ $class }}"
...
$(function() {
    ...
    // Initialize datepicker with date range restrictions
    if ($.fn.bootstrapMaterialDatePicker) {
        var minDateStr = @json($minDate);
        var maxDateStr = @json($maxDate);
        var config = {
            weekStart: 0,
            time: false,
