...
use App\Http\Responses\Contacts\UpdateAccountOwner as UpdateAccountOwnerResponse;
...
class Contacts extends Controller {
    ...
    /**
     * Update the account owner for a client account.
     * Sets the specified user as the new account owner.
     * @param int $id The user ID to set as account owner
     * @return \Illuminate\Http\Response
     */
    public function updateAccountOwner($id) {
        ...
        // Check user is a client user
        if ($user->type != 'client') {
            abort(409, __('lang.action_not_available'));
        }

        // Check user is not already account owner
        if ($user->account_owner == 'yes') {
            abort(409, __('lang.user_already_account_owner'));
        }

        // Get current account owner
        $current_owner = \App\Models\User::Where('clientid', $user->clientid)
            ->where('account_owner', 'yes')
            ->first();

        // Update current owner
        if ($current_owner) {
            $current_owner->account_owner = 'no';
            $current_owner->save();
        }
        ...
        $contacts = $this->userrepo->search($id);
        $previous_owner_contacts = $current_owner ? $this->userrepo->search($current_owner->id) : null;

        // Response payload
        $payload = [
            'contacts' => $contacts,
            'previous_owner_contacts' => $previous_owner_contacts,
            'clientid' => $user->clientid,
        ];

        return new UpdateAccountOwnerResponse($payload);
    }
