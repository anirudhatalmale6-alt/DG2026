...
<tr id="invoice_{{ $invoice->bill_invoiceid  }}" class="{{ $invoice->pinned_status ?? '' }}">
    ...
    <td class="invoices_col_tableconfig_column_21 {{ config('table.tableconfig_column_21') }} tableconfig_column_21"
        id="invoices_col_amount_{{ $invoice->bill_invoiceid }}" style="text-align: right;">
    ...
    <td class="invoices_col_tableconfig_column_23 {{ config('table.tableconfig_column_23') }} tableconfig_column_23"
        ...
        <span class="label {{ runtimeInvoiceStatusColors($invoice->bill_status, 'label') }}">{{
            runtimeInvoiceStatusTitle($invoice->bill_status) }}</span>
        ...
        $invoice->bill_status != 1 && $invoice->bill_status != 5)
        <!--estimate not viewed-->
        ...
        <span class="list-table-action font-size-inherit">
            ...
            <span class="list-table-action dropdown font-size-inherit">
                ...
                <div class="dropdown-menu" aria-labelledby="listTableAction">
                    ...
                    @php
                        $inv_client_user = \App\Models\User::where('clientid', $invoice->bill_clientid)->where('account_owner', 'yes')->first();
                        $inv_client_email = $inv_client_user ? $inv_client_user->email : 'client';
                    @endphp
                    <a class="dropdown-item confirm-action-info" href="javascript:void(0)"
                        data-confirm-title="{{ cleanLang(__('lang.email_to_client')) }}"
                        data-confirm-text="Send invoice to: &lt;span style='color:#e91e63;'&gt;{{ $inv_client_email }}&lt;/span&gt;"
                    ...
                    <a class="dropdown-item actions-modal-button js-ajax-ux-request reset-target-modal-form"
                        ...
                        data-url="{{ url('/invoices/'.$invoice->bill_invoiceid.'/change-status') }}"
                        data-action-url="{{ url('invoices/'.$invoice->bill_invoiceid.'/change-status') }}"
                        data-loading-target="actionsModalBody" data-action-method="POST"
                        data-modal-title="@lang('lang.change_invoice_status')">
                        @lang('lang.change_invoice_status')</a>
                    @endif
                    <!--overdue invoice reminder-->
                    @if(auth()->user()->role->role_invoices > 1 && $invoice->bill_status == 3)
                    <a class="dropdown-item confirm-action-info" href="javascript:void(0)"
            ...
            <span class="list-table-action">
                <button type="button" title="{{ cleanLang(__('lang.star_invoice')) }}"
                    class="data-toggle-action-tooltip btn btn-outline-default btn-circle btn-sm opacity-4 ajax-request {{ $invoice->is_starred ? 'hidden' : '' }} starred-star-button-{{ $invoice->bill_invoiceid }}"
                    id="starred-star-button-{{ $invoice->bill_invoiceid }}"
                    data-url="{{ url('/starred/togglestatus?action=star&resource_type=invoice&resource_id='.$invoice->bill_invoiceid) }}"
                    data-loading-target="starred-star-button-{{ $invoice->bill_invoiceid }}" data-ajax-type="POST"
            ...
            <!--unstar button-->
            <span class="list-table-action">
                <button type="button" title="{{ cleanLang(__('lang.unstar_invoice')) }}"
                    class="data-toggle-action-tooltip btn btn-outline-default btn-circle btn-sm ajax-request text-warning {{ !$invoice->is_starred ? 'hidden' : '' }} starred-unstar-button-{{ $invoice->bill_invoiceid }}"
                    id="starred-unstar-button-{{ $invoice->bill_invoiceid }}"
                    data-url="{{ url('/starred/togglestatus?action=unstar&resource_type=invoice&resource_id='.$invoice->bill_invoiceid) }}"
                    data-loading-target="starred-unstar-button-{{ $invoice->bill_invoiceid }}" data-ajax-type="POST"
                    data-on-start-submit-button="disable">
                    <i class="sl-icon-star"></i>
                </button>
            </span>
