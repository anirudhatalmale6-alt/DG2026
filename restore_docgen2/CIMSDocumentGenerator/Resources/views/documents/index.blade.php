...
@push('styles')
<style>
/* DocGen Index - Teal Theme consistent with CIMS */
.docgen-header {
    background: linear-gradient(135deg, #f8fdfe 0%, #eaf7f9 100%) !important;
    border-bottom: 2px solid #17A2B8 !important;
}
.docgen-header .card-title {
    color: #0d3d56;
    font-weight: 700;
}
.docgen-header .card-title i {
    color: #17A2B8;
}
.btn-docgen-primary {
...
.btn-docgen-primary:hover {
    background: linear-gradient(135deg, #138496, #117a8b);
    color: #fff;
    transform: translateY(-1px);
}
.table thead th {
    background: linear-gradient(135deg, #f8fdfe, #eaf7f9);
    color: #0d3d56;
    font-weight: 600;
    ...
    <div class="row">
        <div class="col-12">
            <div class="card shadow-sm border-0">
                <div class="card-header d-flex justify-content-between align-items-center docgen-header">
                    <h4 class="card-title mb-0">
                        <i class="fa fa-file-pdf me-2"></i> Document Generator
                    </h4>
                    <div class="d-flex gap-2 align-items-center">
                        <a href="{{ route('cimsdocgen.create') }}" class="btn btn-docgen-primary btn-sm shadow-sm">
                ...
                <div class="card-body">
                    ...
                    <form method="GET" action="{{ route('cimsdocgen.index') }}" id="filterForm">
                        ...
                        <div class="row g-2 mb-2">
                            ...
                            <div class="col-md-3">
                                <select name="period_id" class="form-select form-select-sm" onchange="document.getElementById('filterForm').submit();">
                        ...
                        <div class="row g-2 mb-4">
                            ...
                            <div class="col-md-3 d-flex gap-2">
                                ...
                                @if(request('search') || request('status') || request('template_id') || request('client_id') || request('period_id') || request('period_id'))
                                    <a href="{{ route('cimsdocgen.index', request('tab') ? ['tab' => request('tab')] : []) }}" class="btn btn-sm btn-outline-secondary">
                        ...
                        <div class="col-6 col-md-3">
                            <div class="d-flex align-items-center p-3 rounded-3 border"
                                ...
                                <div>
                                    ...
                                    <div class="fw-bold fs-5 text-dark">{{ $activeCount }}</div>
                                </div>
                            </div>
                        </div>
                        <div class="col-6 col-md-3">
                            <div class="d-flex align-items-center p-3 rounded-3 border"
                                ...
                                <div>
                                    ...
                                    <div class="fw-bold fs-5 text-dark">{{ $emailedCount }}</div>
                                </div>
                            </div>
                        </div>
                        <div class="col-6 col-md-3">
                            <div class="d-flex align-items-center p-3 rounded-3 border"
                                ...
                                <div>
                                    ...
                                    <div class="fw-bold fs-5 text-dark">{{ $thisMonthCount }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- ── Tabs: Generated / Emailed ─────────────────────── --}}
                    <ul class="nav nav-pills mb-3" id="docTabs" role="tablist">
                        <li class="nav-item" role="presentation">
                            <a class="nav-link {{ request('tab', 'generated') === 'generated' ? 'active' : '' }}"
                               href="{{ route('cimsdocgen.index', array_merge(request()->except('tab', 'page'), ['tab' => 'generated'])) }}">
                                <i class="fa fa-file-alt me-1"></i> Generated
                                <span class="badge bg-primary bg-opacity-75 ms-1">{{ $generatedCount }}</span>
                            </a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link {{ request('tab') === 'emailed' ? 'active' : '' }}"
                               href="{{ route('cimsdocgen.index', array_merge(request()->except('tab', 'page'), ['tab' => 'emailed'])) }}">
                                <i class="fa fa-paper-plane me-1"></i> Emailed
                                <span class="badge bg-warning bg-opacity-75 text-dark ms-1">{{ $emailedCount }}</span>
                            </a>
                        </li>
                    </ul>
                            ...
                            <table class="table table-hover table-striped align-middle mb-0">
                                <thead style="background: #f8f9fc;">
                                    <tr>
                                        ...
                                        <th class="text-muted fw-semibold" style="font-size: 0.8rem;">Period</th>
                                ...
                                <tbody>
                                    @foreach($documents as $index => $document)
                                        <tr id="document-row-{{ $document->id }}">
                                            ...
                                            <td>
                                                @if($document->period)
                                                    <span class="badge bg-light text-dark border">{{ $document->period->period_name }}</span>
                                                @else
                                                    <span class="text-muted">--</span>
                                                @endif
                                            </td>
                    ...
                    @else
                        ...
                        <div class="text-center py-5">
                            ...
                            <p class="text-muted small mb-4">
                                @if(request('search') || request('status') || request('template_id') || request('client_id') || request('period_id'))
                                    No documents match your current filters. Try adjusting your search criteria.
                                @elseif(request('tab') === 'emailed')
                                    No emailed documents yet. Documents will appear here after being emailed.
                                @else
                            ...
                            <div class="d-flex gap-2 justify-content-center">
                                @if(request('search') || request('status') || request('template_id') || request('client_id') || request('period_id'))
                                    <a href="{{ route('cimsdocgen.index', request('tab') ? ['tab' => request('tab')] : []) }}" class="btn btn-outline-secondary btn-sm">
...
<div class="modal fade" id="emailDocumentModal" tabindex="-1" aria-labelledby="emailDocumentModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            <div class="modal-header docgen-header">
