...
class Settings {
    ...
    public function handle($request, Closure $next) {
         ...
         */
        if (env('SETUP_STATUS') == 'COMPLETED') {
            $invoice_statuses = \App\Models\InvoiceStatus::orderBy('invoicestatus_id', 'ASC')->get();
            config(['invoice_statuses' => $invoice_statuses]);
            $arr = [];
            foreach ($invoice_statuses as $invoice_statuse) {
                $arr[] = $invoice_statuse->invoicestatus_id;
            }
            config(['invoice_statuses_array' => $arr]);
        }
