...
<div class="container-fluid payroll-wrapper">
    <div class="smartdash-page-header" style="margin-bottom: 20px;">
        ...
        <div style="display:flex;gap:8px;">
            <a href="{{ route('cimspayroll.leave.applications.create') }}" class="btn button_master_add"><i class="fa fa-plus"></i> New Application</a>
    ...
    <div class="row"><div class="col-12">
        <div class="card smartdash-form-card">
            ...
            <div class="card-body">
                <form method="GET" action="{{ route('cimspayroll.leave.applications') }}">
                    <div class="row">
                        <div class="col-md-3">
                ...
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><strong>{{ $app->employee->first_name ?? '' }} {{ $app->employee->last_name ?? '' }}</strong><br><small class="text-muted">#{{ $app->employee->employee_number ?? '' }}</small></td>
                                ...
                                <td style="max-width:200px;">{{ \Illuminate\Support\Str::limit($app->reason, 60) }}</td>
                                <td>
                                    @php
                                        $statusColors = ['pending' => 'warning', 'approved' => 'success', 'rejected' => 'danger', 'cancelled' => 'secondary'];
                                    @endphp
                                    <span class="badge bg-{{ $statusColors[$app->status] ?? 'secondary' }}">{{ ucfirst($app->status) }}</span>
                                    @if($app->status === 'rejected' && $app->rejection_reason)
                                    <br><small class="text-danger">{{ $app->rejection_reason }}</small>
                                    @endif
                                </td>
                                <td>
                                    @if($app->status === 'pending')
                                    <form method="POST" action="{{ route('cimspayroll.leave.applications.approve', $app->id) }}" style="display:inline;" onsubmit="return confirm('Approve this leave?');">
                                        @csrf
                                    ...
                                    <button type="button" class="btn button_master_delete" style="padding:4px 12px;font-size:12px;" onclick="rejectLeave({{ $app->id }})"><i class="fa fa-times"></i> Reject</button>
                                    @endif
                                    @if(in_array($app->status, ['pending', 'approved']))
                                    <form method="POST" action="{{ route('cimspayroll.leave.applications.cancel', $app->id) }}" style="display:inline;" onsubmit="return confirm('Cancel this leave application?');">
                                        @csrf
