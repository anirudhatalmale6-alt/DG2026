...
class PayrollController extends Controller
{
    ...
    {
        return view('cims_payroll::payroll.companies.form', ['company' => null]);
    }
    ...
    public function companyStore(Request $request)
    {
        ...
        PayrollCompany::create($validated);
    ...
    public function companyEdit($id)
    {
        ...
        return view('cims_payroll::payroll.companies.form', compact('company'));
    }
    ...
    {
        $taxYear = $request->get('tax_year', '2027');
    ...
    public function payRunEmployees(Request $request)
    {
        $companyId = $request->get('company_id');
        if (!$companyId) {
        ...
        $employees = PayrollEmployee::where('company_id', $companyId)
            ...
            ->orderBy('last_name')
            ->orderBy('first_name')
            ->get(['id', 'employee_number', 'first_name', 'last_name', 'pay_type', 'basic_salary', 'hourly_rate', 'status']);
    ...
    public function payeCalculator()
    {
        return view('cims_payroll::payroll.paye-calculator');
    }

    public function payeCalculatorCalculate(Request $request)
    {
        $grossMonthly = (float) $request->get('gross', 0);
        $age = (int) $request->get('age', 30);
        $frequency = (int) $request->get('frequency', 12);

        if ($grossMonthly <= 0) {
            return response()->json(['success' => false, 'message' => 'Please enter a valid salary amount.']);
        }
        ...
        if ($brackets->isEmpty()) {
            return response()->json(['success' => false, 'message' => 'No tax brackets found for tax year ' . $taxYear . '. Please seed the tax tables first.']);
        }

        // Check threshold — below threshold = no tax
        $belowThreshold = false;
        foreach ($thresholds as $t) {
            if ($t->age_group === 'below_65' && $age < 65 && $annualIncome < $t->threshold_amount) $belowThreshold = true;
            if ($t->age_group === '65_to_74' && $age >= 65 && $age < 75 && $annualIncome < $t->threshold_amount) $belowThreshold = true;
            if ($t->age_group === '75_and_over' && $age >= 75 && $annualIncome < $t->threshold_amount) $belowThreshold = true;
        }

        // Calculate annual tax from brackets
        $annualTax = 0;
        $activeBracketId = null;
        $bracketLabel = '';
        foreach ($brackets as $b) {
            if ($annualIncome >= $b->min_amount && $annualIncome <= $b->max_amount) {
                $annualTax = $b->base_tax + (($annualIncome - $b->min_amount + 1) * $b->rate / 100);
                $activeBracketId = $b->id;
                $bracketLabel = $b->rate . '% (R ' . number_format($b->min_amount, 0, '.', ' ') . ' – R ' . number_format($b->max_amount, 0, '.', ' ') . ')';
                break;
            }
        }
        ...
        foreach ($rebates as $r) {
            if ($r->rebate_type === 'primary') $primaryRebate = $r->amount;
            elseif ($r->rebate_type === 'secondary' && $age >= 65) $secondaryRebate = $r->amount;
            elseif ($r->rebate_type === 'tertiary' && $age >= 75) $tertiaryRebate = $r->amount;
        }

        $totalRebates = $primaryRebate + $secondaryRebate + $tertiaryRebate;
        $taxAfterRebates = $belowThreshold ? 0 : max(0, $annualTax - $totalRebates);
        $monthlyPaye = round($taxAfterRebates / 12, 2);

        // UIF
        $uifCeiling = 17712.00;
        $uifEmployee = round(min($grossMonthly, $uifCeiling) * 0.01, 2);
        $uifEmployer = $uifEmployee;

        // SDL
        $sdl = round($grossMonthly * 0.01, 2);

        // Totals
        $totalEmployeeDeductions = $monthlyPaye + $uifEmployee;
        $netPay = round($grossMonthly - $totalEmployeeDeductions, 2);
        $totalEmployerContributions = $uifEmployer + $sdl;
        $costToCompany = round($grossMonthly + $totalEmployerContributions, 2);

        // Age label
        $ageLabel = $age < 65 ? 'Under 65' : ($age < 75 ? '65 to 74' : '75 and over');

        // Brackets for display
        $bracketData = [];
        foreach ($brackets as $b) {
            $bracketData[] = [
                'min' => $b->min_amount,
                'max' => $b->max_amount,
                'rate' => number_format($b->rate, 0),
                'base_tax' => $b->base_tax,
                'active' => $b->id === $activeBracketId,
            ];
        }
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'age_label' => $ageLabel,
                'bracket_label' => $bracketLabel ?: 'N/A',
                'annual_tax_before_rebates' => round($annualTax, 2),
                'primary_rebate' => $primaryRebate,
                'secondary_rebate' => $secondaryRebate,
                'tertiary_rebate' => $tertiaryRebate,
                'annual_tax_after_rebates' => round($taxAfterRebates, 2),
