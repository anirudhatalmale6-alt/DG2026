...
$result['client_columns'] = DB::select("SHOW COLUMNS FROM client_master LIKE '%code%'");

// Active clients - use * to discover columns
...
$result['periods'] = DB::table('cims_document_periods')
    ->whereIn('financial_year', [2025, 2026])
...
// First client row sample - all columns
$result['client_sample'] = DB::table('client_master')
    ->where('is_active', 1)
    ->first();
