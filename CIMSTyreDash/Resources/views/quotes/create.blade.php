...
<style>
    ...
    .option-tab-content .btn-remove-option { float: right; }
    .nav-tabs .nav-link { font-weight: 500; }
    .nav-tabs .nav-link.active { border-bottom: 3px solid #17A2B8; font-weight: 600; }
    .nav-tabs .nav-link .badge { font-size: 0.7em; }
    ...
    .selected-entity .btn-clear { font-size: 0.85em; }
</style>
@endpush
...
<div class="container-fluid">
    ...
    <div class="row page-titles mx-0">
        <div class="col-sm-6 p-md-0">
            <div class="welcome-text">
                <h4><i class="fas fa-file-invoice me-2"></i>New Quote</h4>
                <span class="ml-1">TyreDash / Quotes / Create</span>
            </div>
        </div>
    ...
    <form id="quoteForm" method="POST" action="{{ route('cimstyredash.quotes.store') }}">
        @csrf
            ...
            <div class="col-lg-8 quote-builder-left">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row g-3">
                            <div class="col-md-3">
                                ...
                                <input type="text" class="form-control" value="{{ $quoteNumber }}" readonly>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Date <span class="text-danger">*</span></label>
                                <input type="date" name="quote_date" class="form-control" value="{{ old('quote_date', now()->format('Y-m-d')) }}" required>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Valid Until</label>
                                <input type="date" name="valid_until" class="form-control" value="{{ old('valid_until', now()->addDays($validityDays)->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" name="salesman_name" class="form-control" placeholder="Salesman name" value="{{ old('salesman_name') }}">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="selected-entity" id="selectedCustomer">
                            <span class="entity-info" id="selectedCustomerName"></span>
                        ...
                        {{-- Search Input --}}
                        <div class="autocomplete-wrapper" id="customerSearchWrapper">
                        ...
                        <div class="quick-add-panel" id="customerQuickAdd">
                            ...
                            <small class="text-muted mt-1 d-block">Customer will be saved when the quote is submitted.</small>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="selected-entity" id="selectedVehicle">
                            <span class="entity-info" id="selectedVehicleName"></span>
                        ...
                        {{-- Search Input --}}
                        <div class="autocomplete-wrapper" id="vehicleSearchWrapper">
                        ...
                        <div class="quick-add-panel" id="vehicleQuickAdd">
                            ...
                            <small class="text-muted mt-1 d-block">Vehicle will be saved when the quote is submitted.</small>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <ul class="nav nav-tabs" id="quoteOptionTabs" role="tablist">
                            {{-- Tabs are dynamically populated by JS --}}
                        </ul>
                        <div class="tab-content option-tab-content mt-3" id="quoteOptionTabContent">
                            <div id="noOptionsMsg" class="text-center text-muted py-4">
                                <i class="fas fa-plus-circle fa-2x mb-2 d-block"></i>
                                Use the Size Search above to add tyre options.
                            </div>
                        </div>

                        {{-- Hidden inputs container for options --}}
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div id="servicesContainer">
                            {{-- Service rows added dynamically --}}
                        </div>
                        <div id="noServicesMsg" class="text-center text-muted py-3">
                            <i class="fas fa-info-circle me-1"></i> No services added yet.
                        </div>

                        {{-- Hidden inputs container for services --}}
                    ...
                    <div class="card summary-card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="customer_order_ref" class="form-control" placeholder="PO / reference number" value="{{ old('customer_order_ref') }}">
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Customer Comment</label>
                                <textarea name="customer_comment" class="form-control" rows="2" placeholder="Visible to customer...">{{ old('customer_comment') }}</textarea>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Internal Notes</label>
                                <textarea name="internal_notes" class="form-control" rows="2" placeholder="Internal only...">{{ old('internal_notes') }}</textarea>
                            </div>
                            ...
                            <div class="d-grid gap-2">
                                ...
                                <button type="button" class="btn btn-primary btn-lg" onclick="submitQuote('send')">
...
(function() {
    ...
    // ---- State ----
    let options = [];   // Array of option objects
    let services = [];  // Array of service objects
                ...
                if (data.vehicles && data.vehicles.length) {
                    data.vehicles.forEach(v => {
                        ...
                        div.innerHTML = '<div class="ac-primary">' + escHtml(v.display_name || v.registration) + '</div>' +
                                        '<div class="ac-secondary">' + escHtml(v.make || '') + ' ' + escHtml(v.model || '') +
                                        (v.current_tyre_size ? ' | Size: ' + escHtml(v.current_tyre_size) : '') + '</div>';
    ...
    function selectVehicle(v) {
        ...
        vehicleResults.style.display = 'none';
        vehicleSearch.value = '';
        // Auto-populate size search if vehicle has a current tyre size
        if (v.current_tyre_size) {
            document.getElementById('sizeSearchInput').value = v.current_tyre_size;
        }
        // Auto-link customer if vehicle has one and no customer selected
        if (v.customer_id && !customerIdInput.value) {
            customerIdInput.value = v.customer_id;
            selectedCustomerName.textContent = v.customer_name || 'Customer #' + v.customer_id;
            selectedCustomerEl.classList.add('show');
            customerSearchWrapper.style.display = 'none';
        }
    }
    ...
    window.toggleQuickAdd = function(type) {
        const panel = document.getElementById(type + 'QuickAdd');
    ...
    function performSizeSearch() {
        ...
        .catch(err => {
            console.error('Size search error:', err);
            sizeResultsContainer.style.display = 'block';
            sizeNoResults.style.display = 'block';
    ...
    function renderOptionTabs() {
        ...
        tabsEl.innerHTML = '';
        // Remove all option panes (keep noOptionsMsg)
        ...
        return '<div class="option-details">' +
            ...
            '<div class="row g-2">' +
                '<div class="col-md-2">' +
                    ...
                    '<input type="number" class="form-control form-control-sm opt-qty" data-idx="' + idx + '" value="' + opt.quantity + '" min="1" max="20">' +
                '</div>' +
                '<div class="col-md-2">' +
                    '<label class="form-label">Unit Cost</label>' +
                    '<input type="number" class="form-control form-control-sm opt-cost" data-idx="' + idx + '" value="' + opt.unit_cost.toFixed(2) + '" step="0.01" min="0" readonly>' +
                '</div>' +
                '<div class="col-md-2">' +
                    '<label class="form-label">Unit Price</label>' +
                    '<input type="number" class="form-control form-control-sm opt-price" data-idx="' + idx + '" value="' + opt.unit_price.toFixed(2) + '" step="0.01" min="0">' +
                '</div>' +
                '<div class="col-md-2">' +
                    '<label class="form-label">Markup %</label>' +
                    '<input type="number" class="form-control form-control-sm opt-markup" data-idx="' + idx + '" value="' + opt.markup_pct.toFixed(1) + '" step="0.1" min="0" max="999">' +
                '</div>' +
                '<div class="col-md-2">' +
                    '<label class="form-label">Discount %</label>' +
                    '<input type="number" class="form-control form-control-sm opt-discount" data-idx="' + idx + '" value="' + opt.discount_pct.toFixed(1) + '" step="0.1" min="0" max="100">' +
                '</div>' +
                '<div class="col-md-2">' +
                    '<label class="form-label">Line Total</label>' +
                    '<input type="text" class="form-control form-control-sm fw-bold opt-line-total" data-idx="' + idx + '" value="' + formatMoney(opt.line_total) + '" readonly>' +
                '</div>' +
            '</div>' +
        '</div>';
    }
    ...
    function bindOptionEvents(idx) {
        ...
        if (removeBtn) removeBtn.addEventListener('click', () => {
            ...
            // If removed was selected, select first
            if (options.length > 0 && !options.some(o => o.is_selected)) {
    ...
    function updateOptionCalc(idx) {
        ...
        // Update service quantities if they are per-tyre
    ...
    addServiceBtn.addEventListener('click', function() {
        ...
        // Prevent duplicate
        if (services.some(s => s.service_id === serviceId)) {
        ...
        services.forEach((svc, idx) => {
            ...
            row.innerHTML = '<div class="flex-grow-1"><strong>' + escHtml(svc.name) + '</strong> <small class="text-muted">(' + escHtml(svc.code) + ')</small></div>' +
                '<div style="width:80px;">' +
                    '<label class="form-label mb-0" style="font-size:0.75em;">Qty</label>' +
                    '<input type="number" class="form-control form-control-sm svc-qty" data-idx="' + idx + '" value="' + svc.quantity + '" min="1" max="100">' +
                '</div>' +
                '<div style="width:120px;">' +
                    '<label class="form-label mb-0" style="font-size:0.75em;">Unit Price</label>' +
                    '<input type="number" class="form-control form-control-sm svc-price" data-idx="' + idx + '" value="' + svc.unit_price.toFixed(2) + '" step="0.01" min="0">' +
                '</div>' +
                '<div style="width:100px;">' +
                    '<label class="form-label mb-0" style="font-size:0.75em;">Total</label>' +
                    '<input type="text" class="form-control form-control-sm fw-bold svc-total" value="' + formatMoney(svc.line_total) + '" readonly>' +
                '</div>' +
    ...
    function recalculateTotals() {
        ...
        // Note: Prices in the system are typically VAT-inclusive.
        // We display the breakdown for clarity.
    ...
    window.submitQuote = function(action) {
        ...
        if (!quoteDate) {
            alert('Please select a quote date.');
            return;
        }

        if (options.length === 0) {
            if (!confirm('No tyre options added. Save quote without options?')) return;
        }

        // Make sure hidden inputs are up to date
        updateOptionHiddenInputs();
        updateServiceHiddenInputs();
