...
namespace Modules\ClientMasterNew\Http\Controllers;

use App\Http\Controllers\Controller;
use Modules\ClientMasterNew\Models\Address;
use Modules\ClientMasterNew\Models\CimsAddressType;
use Modules\ClientMasterNew\Models\CimsBankAccountType;
use Modules\ClientMasterNew\Models\CimsDirectorStatus;
use Modules\ClientMasterNew\Models\CimsDirectorType;
use Modules\ClientMasterNew\Models\ClientMaster;
use Modules\ClientMasterNew\Models\ClientMasterAddress;
use Modules\ClientMasterNew\Models\ClientMasterAudit;
use Modules\ClientMasterNew\Models\ClientMasterBank;
use Modules\ClientMasterNew\Models\ClientMasterDirector;
use Modules\ClientMasterNew\Models\ClientMasterDocument;
use Modules\ClientMasterNew\Models\ClientMasterLookup;
use Modules\ClientMasterNew\Models\ClientPosition;
use Modules\ClientMasterNew\Models\ClientTitle;
use Modules\ClientMasterNew\Models\CompanyType;
use Modules\ClientMasterNew\Models\Document;
use Modules\ClientMasterNew\Models\DocumentType;
use Modules\ClientMasterNew\Models\Person;
use Modules\ClientMasterNew\Models\RefBank;
use Modules\ClientMasterNew\Models\ShareType;
use Modules\ClientMasterNew\Models\CimsVatCycle;
use Carbon\Carbon;
use DB;
...
class ClientMasterController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('clientmasternew::clientmaster.index', compact('clients', 'deletedClients', 'stats'));
    }
    ...
    public function create()
    {
        ...
        return view('clientmasternew::clientmaster.clientmaster_create', compact('lookups', 'clientCode', 'addresses', 'share_types', 'vat_cycles', 'client_titles', 'client_positions', 'banks', 'existingBanks', 'existingAddresses', 'accountTypes', 'address_types', 'persons', 'director_types', 'director_statuses'));
    }
    ...
    public function show($id)
    {
        ...
        return view('clientmasternew::clientmaster.show', compact('client', 'lookups'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('clientmasternew::clientmaster.clientmaster_create', compact('client', 'lookups', 'addresses', 'share_types', 'vat_cycles', 'client_titles', 'client_positions', 'banks', 'existingBanks', 'existingAddresses', 'address_types', 'persons', 'director_types', 'director_statuses', 'existingDirectors'));
    }
    ...
    public function audit($id)
    {
        ...
        return view('clientmasternew::clientmaster.audit', compact('client', 'audits'));
    }
    ...
    private function fillClientData(ClientMaster $client, Request $request)
    {
        ...
        $client->last_vat_return = $this->convertDate($request->last_vat_return);
    ...
    private function handleDocumentUpload(
        ...
        if (! $documentType) {
            \Log::error("Document type not found for field '{$fieldName}' - skipping upload");
            return false;
        }

        $file = $request->file($fieldName);
        if (! $file->isValid()) {
            \Log::error("{$documentType->name} - File upload error: " . $file->getErrorMessage());
            throw new \Exception("{$documentType->name} upload failed: " . $file->getErrorMessage());
        }

        \Log::info("{$documentType->name} - File detected for client: {$client->client_code}, size: " . $file->getSize() . ", mime: " . $file->getMimeType());

        try {
            $document = $this->uploadDocument($client, $file, $documentType);
            ...
            \Log::info("{$documentType->name} - SUCCESS, document id: {$document->id}");
