...
use App\Http\Responses\Invoices\ChangeStatusResponse;
...
class Invoices extends Controller {
    ...
    public function __construct(
        ...
        //Filtering
        $this->middleware('invoicesMiddlewareFiltering')->only([
            'index',
        ]);
        ...
        //Middleware
        $this->middleware('App\Http\Middleware\Invoices\Store')->only([
            'store',
        ]);
    ...
    public function create(CategoryRepository $categoryrepo) {
        ...
        //system default due days
        $invoice_due_days = config('system.settings2_invoices_due_days', 7);
        $invoice_due_days_text = null;

        //check if creating invoice from client profile
        if (request('invoiceresource_type') == 'client' && request()->filled('invoiceresource_id')) {
            //fetch the client
            if ($client = \App\Models\Client::where('client_id', request('invoiceresource_id'))->first()) {
                //use client's specific days or fallback to system default
                $invoice_due_days = $client->client_billing_invoice_due_days ?? config('system.settings2_invoices_due_days', 7);
                //create display text
                $day_label = ($invoice_due_days != 1) ? __('lang.days') : __('lang.day');
                $invoice_due_days_text = __('lang.invoice_date_plus') . ' ' . $invoice_due_days . ' ' . $day_label;
            }
        }
        ...
        $payload = [
            ...
            'settings2_invoices_due_days' => config('system.settings2_invoices_due_days', 7),
            'invoice_due_days' => $invoice_due_days,
            'invoice_due_days_text' => $invoice_due_days_text,
        ];
    ...
    /**
     * Get client's invoice due days
     * @param int $client_id
     * @return \Illuminate\Http\Response
     */
    public function getClientDueDays($client_id) {
        ...
        //get due days (client specific or system default)
        $due_days = $client->client_billing_invoice_due_days ?? config('system.settings2_invoices_due_days', 7);
    ...
    public function store(InvoiceStoreUpdate $request, ClientRepository $clientrepo) {
        ...
        //fire pre-storage event for manual module validation
        event(new \App\Events\Invoices\InvoiceStoring(request()->all(), $request->validated()));
        ...
        //calculate due date if automatic mode
        if (request('invoice_due_date_method') == 'set_due_date_automatically') {
            //get the client
            if (!$client = \App\Models\Client::where('client_id', request('bill_clientid'))->first()) {
                abort(409, __('lang.client_not_found'));
            }

            //determine due days (client specific or system default)
            $due_days = $client->client_billing_invoice_due_days ?? config('system.settings2_invoices_due_days', 7);

            //calculate due date
            $due_date = \Carbon\Carbon::parse(request('bill_date'))
                ->addDays($due_days)
                ->format('Y-m-d');

            //merge due date into request
            request()->merge([
        ...
        $invoice = \App\Models\Invoice::find($bill_invoiceid);

        //set deafult tax mode
        $invoice->bill_tax_type = config('system.settings2_invoices_default_mode') ?? 'summary';
        $invoice->save();
        
        //fire post-storage event for module data saving
        event(new \App\Events\Invoices\InvoiceStored($bill_invoiceid, $invoice, request()->all()));
    ...
    public function saveInvoice(InvoiceSave $request, $id) {
        ...
        //fire pre-update event for stock validation
        event(new \App\Events\Invoices\InvoiceUpdating($invoice, request()->all()));
        ...
        event(new \App\Events\Invoices\InvoiceUpdated($invoice));
    ...
    public function publishInvoice(PublishInvoiceRepository $publishrepo, $id) {
        ...
        //fire pre-publishing event
        event(new \App\Events\Invoices\InvoicePublishing(request(), $id));
        ...
        //fire post-publishing event
        event(new \App\Events\Invoices\InvoicePublished($result['invoice']));
    ...
    public function publishScheduledInvoice($id) {
        ...
        //fire pre-scheduling event
        event(new \App\Events\Invoices\InvoiceScheduling(request(), $id));
        ...
        //fire post-scheduling event
        event(new \App\Events\Invoices\InvoiceScheduled(request(), $id, $payload));
    ...
    public function resendInvoice($id) {
        ...
        //fire post-resend event
        event(new \App\Events\Invoices\InvoiceResent(request(), $id, $payload));
    ...
    public function destroy(DestroyRepository $destroyrepo) {
        ...
        //collect invoice IDs for pre-delete event
        $invoice_ids = [];
        ...
        event(new \App\Events\Invoices\InvoiceDeleting(request(), $invoice_ids));
        ...
        foreach (request('ids') as $id => $value) {
            ...
            if ($value == 'on') {
                ...
                // Broadcast invoice destroyed event
                event(new \App\Events\Invoices\InvoiceDestroyed($id));
            }
        }
        //reponse payload
        $payload = [
    ...
    public function changeCategoryUpdate(CategoryRepository $categoryrepo) {
        ...
        foreach (request('ids') as $bill_invoiceid => $value) {
            if ($value == 'on') {
                ...
                //add to array
                $allrows[] = $invoices;
                $updated_invoice_ids[] = $bill_invoiceid;
            }
        }
        ...
        //fire post-category changed event
        event(new \App\Events\Invoices\InvoiceCategoryChanged(request(), $updated_invoice_ids, $payload));
    ...
    public function attachProjectUpdate() {
        ...
        //fire post-project attached event
        event(new \App\Events\Invoices\InvoiceProjectAttached(request(), request()->route('invoice'), request('attach_project_id'), $payload));
    ...
    public function dettachProject() {
        ...
        //fire post-project detached event
        event(new \App\Events\Invoices\InvoiceProjectDetached(request(), request()->route('invoice'), $payload));
    ...
    public function storeClone(InvoiceClone $request, CloneInvoiceRepository $cloneinvoicerepo, $id) {
        ...
        //fire pre-cloning event
        event(new \App\Events\Invoices\InvoiceCloning(request(), $id));
        ...
        //fire post-cloning event
        event(new \App\Events\Invoices\InvoiceCloned(request(), $id, $invoice_id, $payload));
    ...
    public function emailClient($id) {
        ...
        //fire post-email event
        event(new \App\Events\Invoices\InvoiceEmailed(request(), $id, $payload));
    ...
    public function overdueReminder($id) {
        ...
        //fire pre-reminder event
        event(new \App\Events\Invoices\InvoiceOverdueReminding(request(), $id));
        ...
        //fire post-reminder event
        event(new \App\Events\Invoices\InvoiceOverdueReminded(request(), $id, $payload));
    ...
    public function recurringSettings($id) {
        ...
        //get the project
        $invoice = \App\Models\Invoice::with('lineitems')->Where('bill_invoiceid', $id)->first();
    ...
    public function recurringSettingsUpdate(InvoiceRecurrringSettings $request, $id) {
        ...
        if ($invoice->bill_publishing_type == 'scheduled' && $invoice->bill_status == 1) {
        ...
        //fire pre-recurring update event
        event(new \App\Events\Invoices\InvoiceRecurringUpdating(request(), $id));
        ...
        //update line item recurring statuses
        $lineitems = \App\Models\Lineitem::where('lineitemresource_type', 'invoice')
            ->where('lineitemresource_id', $invoice->bill_invoiceid)
            ->get();

        $submitted_recurring_items = request('lineitem_recurring_status') ?? [];

        foreach ($lineitems as $lineitem) {
            if (isset($submitted_recurring_items[$lineitem->lineitem_id])) {
                $lineitem->lineitem_recurring_status = 'recurring';
            } else {
                $lineitem->lineitem_recurring_status = 'non-recurring';
            }
            $lineitem->save();
        }
        ...
        //fire post-recurring update event
        event(new \App\Events\Invoices\InvoiceRecurringUpdated(request(), $id, $payload));
    ...
    public function stopRecurring() {
        ...
        //fire post-recurring stopped event
        event(new \App\Events\Invoices\InvoiceRecurringStopped(request(), request()->route('invoice'), $payload));
    ...
    public function attachFiles(Request $request, FileRepository $filerepo, $id) {
        ...
        //fire pre-attach event
        event(new \App\Events\Invoices\InvoiceFileAttaching(request(), $id));
        ...
        if ($file = $request->file('file')) {
            ...
            //fire post-attach event
            event(new \App\Events\Invoices\InvoiceFileAttached(request(), $uniqueid, $id, $payload));
    ...
    public function deleteFile() {
        ...
        //fire post-file deleted event
        event(new \App\Events\Invoices\InvoiceFileDeleted(request(), request('file_uniqueid'), $payload));
    }
    ...
    public function updateTaxType($id) {
        ...
        //fire post-tax type updated event
        event(new \App\Events\Invoices\InvoiceTaxTypeUpdated(request(), $id, $payload));
    ...
    public function togglePinning(PinnedRepository $pinrepo, $id) {
        ...
        //fire post-pinned event
        event(new \App\Events\Invoices\InvoicePinned(request(), $id, $payload));
    ...
    public function bulkDettachFromProject() {
        ...
        foreach (request('ids') as $invoice_id => $value) {
            if ($value == 'on') {
                ...
                if ($invoice = \App\Models\Invoice::Where('bill_invoiceid', $invoice_id)->first()) {
                    ...
                    //add to array
                    $allrows[] = $invoices;
                    $detached_invoice_ids[] = $invoice_id;
                }
            }
        }
        ...
        //fire post-bulk detached event
        event(new \App\Events\Invoices\InvoiceBulkProjectDetached(request(), $detached_invoice_ids, $payload));
    ...
    public function bulkEmailClient() {
        ...
        //collect invoice IDs for pre-email event
        $invoice_ids = [];
        foreach (request('ids') as $bill_invoiceid => $value) {
            if ($value == 'on') {
                $invoice_ids[] = $bill_invoiceid;
            }
        }

        //fire pre-bulk emailing event
        event(new \App\Events\Invoices\InvoiceBulkEmailing(request(), $invoice_ids));

        //process all selected invoices
        $allrows = array();
        $emailed_invoice_ids = array();
        foreach (request('ids') as $bill_invoiceid => $value) {
            if ($value == 'on') {
                //validate the invoice exists
                ...
                //add to array
                $allrows[] = $invoices;
                $emailed_invoice_ids[] = $bill_invoiceid;
            }
        }
        ...
        //fire post-bulk emailed event
        event(new \App\Events\Invoices\InvoiceBulkEmailed(request(), $emailed_invoice_ids, $payload));
    ...
    /**
     * Show the form for changing invoice status
     * @param int $id invoice id
     * @return \Illuminate\Http\Response
     */
    public function changeStatus($id) {
        ...
        $statuses = \App\Models\InvoiceStatus::orderBy('invoicestatus_position', 'asc')->get();
     ...
     * @return \Illuminate\Http\Response
     */
    public function changeStatusUpdate($id) {

        //validate
        $validator = Validator::make(request()->all(), [
            'bill_status' => 'required|exists:invoice_statuses,invoicestatus_id',
        ]);
        ...
        //update the status and set trigger to manual
        $invoice->bill_status = request('bill_status');
        $invoice->bill_status_trigger = 'manual';
        $invoice->save();
