...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        //if we are filtering with remember checked, redirect to reload page
        if (request('query-type') == 'filter' && request()->filled('filter_remember')) {
            $jsondata['redirect_url'] = url('/invoices');
            return response()->json($jsondata);
        }

        //fire event
        event(new \App\Events\Invoices\Responses\InvoiceIndex($request, $this->payload));
