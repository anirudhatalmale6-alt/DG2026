...
function NXbootstrap($self, action) {
    ...
    $(document).ready(function () {
        ...
        //basic select2 - with search box; To keep the drowdown open add data-keep-open="true" in the html
        $(".select2-basic-with-search").each(function () {
            $(this).select2({
                minimumResultsForSearch: 1,
                closeOnSelect: $(this).data('keep-open') !== true
            });
        });
        ...
        buttons: {
            ...
            yes: {
                ...
                action: function () {
                    ...
                    if ($('#confirm_popup_checkbox').length && $('#confirm_popup_checkbox').is(':checked')) {
                        var checkbox_field_id = $('#confirm_popup_checkbox').attr('data-confirm-checkbox-field-id');
                        //get current URL
                        var current_url = $self.attr('data-url');
                        //add checkbox value as query parameter
                        var separator = current_url.indexOf('?') !== -1 ? '&' : '?';
                        $self.attr('data-url', current_url + separator + checkbox_field_id + '=yes');
                    }
                    //fast delete ux
