...
function nxAjaxUxRequest(obj, virtualPostArray = {}) {
		...
		if (obj.attr('type') == 'submit' || obj.attr('data-type') == 'search' || obj.attr('data-type') == 'form' || obj.attr('data-type') == 'PUT' || obj.attr('data-ajax-type') == 'post') {
			...
			form.find("input, textarea, select").each(function () {
				...
				if ($(this).attr('data-type') == 'ckeditor') {
					...
					//handel checkboxes. Both plain/single checkboxes, name='foo' and arrays, name="foo[]" and name="foo[bar]"
					if ($(this).is(':checkbox')) {
						//check if this is a checkbox group (field name ends with [])
						if (field_name && /\[\]$/.test(field_name)) {
							//for checkbox groups, collect checked values into an array
							if ($(this).prop('checked')) {
								//initialize array if it doesn't exist
								if (!Array.isArray(NXAJAX.post[field_name])) {
									NXAJAX.post[field_name] = [];
								}
								//add the actual value (not 'on')
								NXAJAX.post[field_name].push($(this).val());
							}
							//skip adding to post object below (already handled)
							return true; //continue to next iteration
						} else {
							//for single checkboxes and associative array checkboxes, use existing behavior
							if ($(this).prop('checked')) {
								var field_value = 'on';
							} else {
								var field_value = '';
	...
	NXAJAX.getPayload = function (obj) {
		...
		NXAJAX.payload.file_download = obj.file_download;
	 ...
	 * Handles file download responses from the backend
	 *
	 * [BACKEND RESPONSE FORMAT]
	 *    ['file_download']['url'] = 'http://domain.com/download/file'
	 *    ['file_download']['filename'] = 'report.pdf'
	 *
	 *------------------------------------------------------------------------------------------------------------------------------------*/
	NXAJAX.handleFileDownload = function () {

		//get the payload
		var payload = NXAJAX.payload.file_download;

		//debug
		NXAJAX.log('[ajax] handleFileDownload() - triggering file download - [payload]:', payload);

		//trigger download if payload exists
					...
					if (value.selector != undefined && value.action != undefined && value.value != undefined) {
						...
						if (value.action == 'replace-with') {
							...
							if ($element.length > 0) {
								...
								// FALLBACK: Try direct DOM API
								if (value.selector.startsWith('#')) {
									var element = document.getElementById(value.selector.substring(1));
									if (element) {
										element.outerHTML = value.value;
									}
								}
							}
						}
