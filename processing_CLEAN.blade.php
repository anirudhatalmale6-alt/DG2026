...
.pp-container { display: flex; height: calc(100vh - 220px); overflow: hidden; background: #f4f6f9; border-radius: 8px; border: 1px solid #e0e0e0; box-shadow: 0 2px 8px rgba(0,0,0,0.06); }
...
.pp-control-bar label { font-size: 12px; opacity: 0.85; margin-bottom: 2px; display: block; }
.pp-control-bar select {
    background: rgba(255,255,255,0.15); border: 1px solid rgba(255,255,255,0.3);
    color: #fff; padding: 6px 12px; border-radius: 6px; font-size: 13px; min-width: 280px;
}
.pp-control-bar select option { color: #333; background: #fff; }
.pp-control-bar select:disabled { opacity: 0.5; cursor: not-allowed; }
.pp-load-btn {
    background: rgba(255,255,255,0.2); border: 1px solid rgba(255,255,255,0.4);
    color: #fff; padding: 6px 18px; border-radius: 6px; font-size: 13px;
    font-weight: 600; cursor: pointer; transition: all 0.2s; display: inline-flex;
    align-items: center; gap: 6px; white-space: nowrap;
}
.pp-load-btn:hover { background: rgba(255,255,255,0.3); }
.pp-load-btn:disabled { opacity: 0.4; cursor: not-allowed; }
.pp-period-info { margin-left: auto; font-size: 12px; opacity: 0.9; text-align: right; }
.pp-period-info strong { font-size: 14px; display: block; }
...
<div class="pp-control-bar">
    ...
    <div>
        ...
        <select id="ppTaxYearSelect" onchange="ppOnTaxYearChange()" disabled style="min-width:160px;">
            <option value="">— Select —</option>
        </select>
    </div>
    <div>
        <label>Pay Period</label>
        <select id="ppPeriodSelect" onchange="ppOnPeriodChange()" disabled style="min-width:180px;">
            <option value="">— Select —</option>
        </select>
    </div>
...
function ppOnCompanyChange() {
    ...
    if (!companyId) {
        taxYearSelect.innerHTML = '<option value="">— Select —</option>';
    ...
    for (let y = currentTaxYear + 1; y >= currentTaxYear - 2; y--) {
        ...
        taxYearSelect.innerHTML += '<option value="' + y + '"' + selected + '>' + (y - 1) + ' / ' + y + '</option>';
    }
...
function ppOnTaxYearChange() {
    ...
    ppMonths.forEach(m => {
        ...
        periodSelect.innerHTML += '<option value="' + val + '">' + m.label + ' ' + calYear + '</option>';
    });

    periodSelect.disabled = false;
    if (preselect) periodSelect.value = preselect;
    ppOnPeriodChange();
}
...
function ppOnPeriodChange() {
    ...
    loadBtn.disabled = !(ppSelectedPeriod && document.getElementById('ppCompanySelect').value && document.getElementById('ppTaxYearSelect').value);

    // Update period display
    const display = document.getElementById('ppPeriodDisplay');
    if (display && ppSelectedPeriod && ppSelectedTaxYear) {
        const sel = periodSelect.options[periodSelect.selectedIndex];
        const yr = parseInt(ppSelectedTaxYear);
        display.innerHTML = '<strong><i class="fas fa-calendar-alt"></i> ' + (sel ? sel.text : '') + '</strong><span>Tax Year: ' + (yr - 1) + '/' + yr + '</span>';
    }
}
