      ...
      $.ajax({
        ...
        if (jsonresp.stat === 1) {
          ...
          $(".pxl-form-upload-demo").on("click", ".btn-upload", function () {
            ...
            var nonce = $('#pxlart_upload_demo_manual_nonce').val();
            var form_data = new FormData();
            form_data.append("file", file_data);
            form_data.append("action", "pxlart_upload_demo_manual");
            form_data.append("demo_id", id);
            form_data.append("pxlart_upload_demo_manual_nonce", nonce);
            $.ajax({
              url: pxl_ajax_url,
              type: "post",
              contentType: false,
              ...
              if (res == "1") {
                ...
                $(".pxl-demo-error-confirm")
                  ...
                  .append('<p class="err">File upload incorect or not found</p>');
              } else if (res == "3") {
                $(".pxl-demo-error-confirm")
                  .find(".pxl-form-upload-demo")
                  .append(
                ...
                $(".pxl-demo-error-confirm")
                  .find(".pxl-form-upload-demo")
                  .append('<p class="err">File upload is false!</p>');
              }
            });
          });
        }
    ...
    options[count] &&
      ajaxRun(
        ...
        function () {
          count++;
          options[count] &&
            ajaxRun("pxlart_" + options[count], options, id, count, crop_img, function () {
              count++;
              options[count] &&
                ajaxRun("pxlart_" + options[count], options, id, count, crop_img, function () {
                  count++;
                      ...
                      options[count] &&
                        ajaxRun(
                          ...
                          function () {
                            count++;
                            options[count] &&
                              ajaxRun("pxlart_" + options[count], options, id, count, crop_img);
                          }
                        );
                    });
                });
