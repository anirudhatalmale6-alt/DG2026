...
Route::group(['prefix' => 'clients'], function () {
    Route::any("/search", "Clients@index")->name('clients.search');
    ...
    Route::any("/{client}/{section}", "Clients@showDynamic")
        ->where([
            'client' => '[0-9]+',
            'section' => 'details|contacts|projects|files|client-files|tickets|invoices|expenses|payments|timesheets|estimates|notes|project-files|client-files|contracts|proposals']);
});
...
Route::group(['prefix' => 'contacts'], function () {
    ...
    Route::post("/{contact}/update-account-owner", "Contacts@updateAccountOwner")->middleware(['demoModeCheck']);
});
...
Route::group(['prefix' => 'invoices'], function () {
    ...
    Route::get("/client-due-days/{client_id}", "Invoices@getClientDueDays")->where('client_id', '[0-9]+');
    ...
    Route::get("/{id}/change-status", "Invoices@changeStatus")->where('id', '[0-9]+');
    Route::post("/{id}/change-status", "Invoices@changeStatusUpdate")->where('id', '[0-9]+');
...
Route::group(['prefix' => 'estimates'], function () {
    ...
    Route::post("/{estimate}/change-tax-type", "Estimates@updateTaxType");
...
Route::group(['prefix' => 'payments'], function () {
    Route::any("/search", "Payments@index")->name('payments.search');
...
Route::group(['prefix' => 'tickets'], function () {
    ...
    Route::post('/{ticket}/close', 'Tickets@closeTicket')->where('ticket', '[0-9]+');
...
Route::group(['prefix' => 'proposals'], function () {
    ...
    Route::get("/{proposal}/attach-project", "Proposals@attachProject")->where('proposal', '[0-9]+');
    Route::post("/{proposal}/attach-project", "Proposals@attachProjectUpdate")->where('invoice', '[0-9]+');
    Route::get("/{proposal}/detach-project", "Proposals@dettachProject")->where('invoice', '[0-9]+');
});
...
Route::group(['prefix' => 'contracts'], function () {
    ...
    Route::get("/{contract}/edit-automation", "Contracts@editAutomation")->where('contract', '[0-9]+');
    Route::put("/{contract}/update-automation", "Contracts@updateAutomation")->where('contract', '[0-9]+');
    ...
    Route::get("/view/{contract}/sign", "Contracts@signGuest");
    Route::post("/view/{contract}/sign", "Contracts@signGuestAction");
    ...
    Route::get("/{contract}/sign/client-in-person", "Contracts@signClientInPerson");
    Route::post("/{contract}/sign/client-in-person", "Contracts@signClientInPersonAction");
...
Route::group(['prefix' => 'settings/invoices'], function () {
    ...
    Route::get("/statuses", "Settings\Invoices@statuses");
    Route::get("/statuses/{id}/edit", "Settings\Invoices@editStatus")->where('id', '[0-9]+');
    Route::put("/statuses/{id}", "Settings\Invoices@updateStatus")->where('id', '[0-9]+')->middleware(['demoModeCheck']);
    Route::get("/statuses/create", "Settings\Invoices@createStatus");
    Route::post("/statuses/create", "Settings\Invoices@storeStatus");
    Route::get("/move/{id}", "Settings\Invoices@move")->where('id', '[0-9]+');
    Route::put("/move/{id}", "Settings\Invoices@updateMove")->where('id', '[0-9]+');
    Route::delete("/statuses/{id}", "Settings\Invoices@destroyStatus")->where('id', '[0-9]+')->middleware(['demoModeCheck']);
    Route::post("/update-stage-positions", "Settings\Invoices@updateStagePositions");
});

//SETTINGS - PRODUCTS
Route::group(['prefix' => 'settings/products'], function () {
    Route::get('/custom-fields', [App\Http\Controllers\Settings\ProductCustomFields::class, 'index']);
...
Route::group(['prefix' => 'settings/units'], function () {
    ...
    Route::get('/create', 'Settings\Units@create');
    Route::post('/create', 'Settings\Units@store');
    Route::get('/{id}/edit', 'Settings\Units@edit')->where('id', '[0-9]+');
    Route::put('/{id}', 'Settings\Units@update')->where('id', '[0-9]+');
    Route::delete('/{id}', 'Settings\Units@destroy')->where('id', '[0-9]+');
});
...
Route::group(['prefix' => 'settings/contracts'], function () {
    ...
    Route::get("/automation", "Settings\Contracts@automation");
    Route::put("/automation", "Settings\Contracts@automationUpdate");
});
...
//IMPORT PROJECTS
Route::resource('import/projects', 'Import\Projects');

//IMPORT ITEMS
Route::resource('import/items', 'Import\Items');
