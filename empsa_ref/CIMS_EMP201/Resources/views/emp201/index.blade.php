...
<div class="container-fluid">
    ...
    <div class="toolbar-wrapper">
                ...
                <li class="nav-item">
                    <a class="nav-link {{ request('status') === 'inactive' ? 'active' : '' }}"
                       href="{{ route('cimsemp201.index', array_merge(request()->except('status', 'page'), ['status' => 'inactive'])) }}">
                        Inactive <span class="badge bg-warning text-dark">{{ $stats['inactive'] }}</span>
                    </a>
                </li>
                <li class="nav-item">
        ...
        <a href="{{ route('cimsemp201.create') }}" class="btn-add-emp201">
    ...
    <div class="table-card">
        <div class="table-card-header">
            ...
            <span class="record-count">{{ $declarations->total() }} record{{ $declarations->total() != 1 ? 's' : '' }}</span>
        </div>
                    ...
                    <tbody>
                        @foreach($declarations as $index => $declaration)
                            ...
                            <tr id="emp201-row-{{ $declaration->id }}">
                                {{-- # --}}
                                <td><span class="row-num">{{ $declarations->firstItem() + $index }}</span></td>

                                {{-- Year End --}}
                                <td><span class="year-badge">{{ $declaration->financial_year }}</span></td>

                                {{-- Client Code --}}
                                <td><span class="client-code-badge">{{ $declaration->client_code ?? '--' }}</span></td>

                                {{-- Reference --}}
                                <td>
                                    @if($declaration->payment_reference)
                                        <span style="font-size:13px; font-weight:600; color:#334155;">{{ $declaration->payment_reference }}</span>
                                    @else
                                ...
                                {{-- Tax Period --}}
                                <td>
                                    @if($declaration->pay_period)
                                        <span class="period-badge">
                                            <i class="fa fa-calendar-day"></i>
                                            {{ $declaration->pay_period }}
                                        </span>
                                    @else
                                        <span class="ref-text">--</span>
                                    @endif
                                ...
                                {{-- PAYE --}}
                                <td class="currency-col {{ $isZeroPaye ? 'zero' : '' }}">{{ $fmtPaye }}</td>

                                {{-- SDL --}}
                                <td class="currency-col {{ $isZeroSdl ? 'zero' : '' }}">{{ $fmtSdl }}</td>

                                {{-- UIF --}}
                                <td class="currency-col {{ $isZeroUif ? 'zero' : '' }}">{{ $fmtUif }}</td>

                                {{-- Liability --}}
                                <td class="currency-col {{ $isZeroLiab ? 'zero' : '' }}">{{ $fmtLiability }}</td>

                                {{-- Penalties --}}
                                <td class="currency-col {{ $isZeroPen ? 'zero' : '' }}">{{ $fmtPenalties }}</td>

                                {{-- Total Due --}}
                                <td class="total-due-col">{{ $fmtTotal }}</td>

                                {{-- Status --}}
                                <td class="text-center">
                                    @if($declaration->status === 'active' || $declaration->status == 1 || $declaration->is_active == 1)
                                        <span class="status-badge active"><span class="dot"></span> Active</span>
                                    @elseif($declaration->status === 'inactive' || $declaration->status == 0 || $declaration->is_active == 0)
                                    ...
                                    @else
                                        <span class="status-badge inactive"><span class="dot"></span> {{ ucfirst($declaration->status ?? 'Unknown') }}</span>
                                    @endif
                                </td>
                                ...
                                <td>
                                    @if($declaration->created_at)
                                        <span class="date-text">{{ $declaration->created_at->format('d M Y') }}</span>
                                    @else
                                        <span class="ref-text">--</span>
                                    @endif
                                </td>
                                ...
                                <td class="text-center">
                                    <div class="dropdown d-inline-block">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end"
                                            ...
                                            <li>
                                                <a class="dropdown-item" href="{{ route('cimsemp201.show', $declaration->id) }}?pdf=1"
                                                   style="border-radius:8px;padding:10px 14px;font-weight:500;font-size:14px;transition:all .15s ease">
    ...
    document.querySelectorAll('.btn-restore-emp201').forEach(function (btn) {
        btn.addEventListener('click', function (e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch('{{ url("cims/emp201") }}/' + empId + '/status', {
                        method: 'PATCH',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                            'Content-Type': 'application/json',
    ...
    document.querySelectorAll('.btn-delete-permanent-emp201').forEach(function (btn) {
        btn.addEventListener('click', function (e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch('{{ url("cims/emp201") }}/' + empId + '/force', {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                            'Content-Type': 'application/json',
    ...
    document.querySelectorAll('.btn-email-emp201').forEach(function (btn) {
        btn.addEventListener('click', function (e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch('{{ url("cims/emp201") }}/' + empId + '/email', {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                            'Content-Type': 'application/json',
