...
class EmailController extends Controller
{
    ...
    public function compose(Request $request)
    {
        ...
        $periods = DB::table('cims_document_periods')
            ...
            ->get(['id', 'period_name', 'tax_year', 'period_combo']);
        ...
        // Pre-select template by name (from external modules like Document Viewer)
        $selectedTemplateId = null;
        if ($request->has('template_name')) {
            $tpl = DB::table('cims_email_templates')
                ->where('name', $request->get('template_name'))
                ->where('is_active', 1)
                ->first();
            if ($tpl) {
        ...
        // Pre-attach document from Document Manager
        $preAttachedDoc = null;
        if ($request->has('attach_document_id')) {
            $preAttachedDoc = DB::table('cims_documents')
                ->where('id', $request->get('attach_document_id'))
                ->first();
            // If document not found in cims_documents, try documents table
            if (!$preAttachedDoc) {
                $preAttachedDoc = DB::table('documents')
                    ->where('id', $request->get('attach_document_id'))
                    ->first();
            }
        }
        ...
        return view('cimscore::partials.cims_master_compose_email', compact(
            'clients', 'templates', 'periods', 'draft', 'selectedClientId', 'counts',
            'signatureHtml', 'disclaimerHtml', 'fromEmail', 'fromName',
            'recentContacts', 'showAll', 'selectedTemplateId', 'preAttachedDoc'
        ));
    }
    ...
    public function send(Request $request)
    {
        ...
        $toEmails = array_values(array_filter(array_map('trim', explode(',', $request->to_emails))));
        $ccEmails = $request->cc_emails ? array_values(array_filter(array_map('trim', explode(',', $request->cc_emails)))) : [];
        $bccEmails = $request->bcc_emails ? array_values(array_filter(array_map('trim', explode(',', $request->bcc_emails)))) : [];
        ...
        $signature = $this->getUserSignature();
        $disclaimerHtml = $this->buildDisclaimerHtml($signature);
        $rawBodyHtml = $request->body_html;
        if ($disclaimerHtml) {
            $rawBodyHtml .= $disclaimerHtml;
        }

        // Wrap in proper HTML email document for consistent rendering across email clients
        $bodyHtml = '<!DOCTYPE html>'
            ...
            . '</head>'
            . '<body style="margin:0;padding:20px;font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#333;background-color:#ffffff;">'
        ...
        // Handle pre-attached documents from Document Manager
        if ($request->has('pre_attached_document_ids')) {
            $docIds = is_array($request->pre_attached_document_ids) ? $request->pre_attached_document_ids : [$request->pre_attached_document_ids];
            foreach ($docIds as $docId) {
                $doc = DB::table('cims_documents')->where('id', $docId)->first();
                if (!$doc) {
                ...
                if ($doc && $doc->file_path) {
                    ...
                    if (file_exists($sourcePath)) {
                        $ext = pathinfo($doc->file_stored_name ?? $doc->file_original_name ?? 'file', PATHINFO_EXTENSION);
                        $filename = Str::random(20) . '.' . $ext;
                        $destDir = storage_path('app/public/email_attachments/' . $emailId);
                        if (!is_dir($destDir)) {
                        ...
                        DB::table('cims_email_attachments')->insert([
                            'email_id' => $emailId,
                            'filename' => $filename,
                            'original_filename' => $doc->file_stored_name ?? $doc->file_original_name ?? 'document.' . $ext,
                            'file_path' => 'email_attachments/' . $emailId . '/' . $filename,
                            'file_size' => filesize($sourcePath),
                            'mime_type' => $doc->file_mime_type ?? $doc->mime_type ?? 'application/octet-stream',
                            'created_at' => now(),
                        ]);
                    }
                }
        ...
        try {
            ...
            $validTo = array_filter($toEmails, function ($e) { return filter_var($e, FILTER_VALIDATE_EMAIL); });
            $validCc = array_filter($ccEmails, function ($e) { return filter_var($e, FILTER_VALIDATE_EMAIL); });
            $validBcc = array_filter($bccEmails, function ($e) { return filter_var($e, FILTER_VALIDATE_EMAIL); });

            \Log::info('CIMS Email sending', [
                'email_id' => $emailId,
                'raw_to' => $request->to_emails,
                'parsed_to' => $validTo,
                'cc' => $validCc,
                'bcc' => $validBcc,
                'subject' => $request->subject,
            ...
            // Send individual email to each TO recipient for reliable delivery
            $sentCount = 0;
            foreach ($validTo as $toEmail) {
                $isFirst = ($sentCount === 0);
                Mail::html($bodyHtml, function ($message) use ($toEmail, $validCc, $validBcc, $fromEmail, $fromName, $request, $attachments, $isFirst) {
                    $message->from($fromEmail, $fromName);
                    $message->to($toEmail);

                    // Only add CC/BCC on the first send to avoid duplicate CC/BCC emails
                    if ($isFirst) {
                        foreach ($validCc as $cc) {
                            $message->cc($cc);
                        }
                        foreach ($validBcc as $bcc) {
                            $message->bcc($bcc);
                        }
                    }
                ...
                $sentCount++;
                \Log::info('CIMS Email delivered to: ' . $toEmail . ' (send #' . $sentCount . ')');
            }
            ...
            $recipientCount = count($validTo) + count($validCc) + count($validBcc);
            return redirect()->route('cimsemail.sent')
                ->with('success', 'Email sent successfully to ' . $recipientCount . ' recipient(s)!');
    ...
    public function view($id)
    {
        ...
        // Look up contact photo from first TO email
        $contactPhoto = null;
        $toList = json_decode($email->to_emails, true) ?? [];
        if (!empty($toList)) {
            $firstEmail = trim($toList[0]);
            $contact = DB::table('cims_master_contacts')
                ...
                ->first(['photo']);
            if ($contact) {
                $contactPhoto = url('/storage/contact_photos/' . $contact->photo);
            }
        }

        return view('cims_email::emails.view', compact('email', 'attachments', 'client', 'counts', 'contactPhoto'));
    }
    ...
    {
        // Auto-seed bank statement templates if they don't exist
        $this->seedBankTemplatesIfNeeded();
    ...
    public function getClientContacts($clientId)
    {
        ...
        // Get client info - use SELECT * to avoid column-not-found issues with OPcache
        $client = DB::table('client_master')
            ->where('client_id', $clientId)
            ->first();

        // Get client address from client_master_addresses -> cims_addresses
        $clientAddress = '';
        try {
            ...
            if ($addrLink && $addrLink->address_id) {
                $addr = DB::table('cims_addresses')
                    ->where('id', $addrLink->address_id)
                    ->first();
                if ($addr) {
                    $clientAddress = $addr->long_address ?? '';
                    if (empty($clientAddress)) {
                        // Build from parts if long_address is empty
                        $parts = array_filter([
                            $addr->street_number ?? '',
                            $addr->street_name ?? '',
                            $addr->suburb ?? '',
        ...
        // Attach address to client object
        if ($client) {
            $client->resolved_address = $clientAddress;
        }
    ...
    {
        try {
            // First try to find signature for the logged-in user
            $sig = DB::table('cims_email_signatures')
                ->where('user_id', Auth::id())
                ->where('is_active', 1)
                ->first();

            // Fallback: if no signature for this user, use the first active signature
            if (!$sig) {
                $sig = DB::table('cims_email_signatures')
                    ->where('is_active', 1)
                    ->first();
    ...
    private function buildSignatureHtml($signature)
    {
        ...
        $html = '<table cellpadding="0" cellspacing="0" style="font-family:Arial,sans-serif;font-size:13px;color:#333;border-collapse:collapse;width:100%;max-width:850px;">';
        $html .= '<tr><td style="vertical-align:top;width:auto;">';
        // Inner signature table
        $html .= '<table cellpadding="0" cellspacing="0" style="font-family:Arial,sans-serif;font-size:13px;color:#333;border-collapse:collapse;width:100%;max-width:650px;">';
        ...
        $html .= '</table>';
        // Close inner signature cell, add stamp cell on the right
        $html .= '</td>';
        ...
        // Full-width row for separator + logo (spans both columns)
        $html .= '<tr><td colspan="2" style="padding-top:14px;"><div style="border-top:1px solid #e0e0e0;"></div></td></tr>';

        // Company logo row - left aligned, spans full width
    ...
    /**
     * Seed bank statement request templates (temporary - remove after use)
     */
    public function seedBankTemplates()
    {
        $now = now();
        ...
        return response()->json(['message' => "Done. Inserted {$inserted} template(s).", 'inserted' => $inserted]);
    }

    /**
     * Silent Send - One-touch background email send (no compose form shown)
     *
     * Called via AJAX/POST from any module in the system.
     * Uses the same email engine: SMTP settings, signature, disclaimer, audit trail.
     * All emails appear in Sent folder just like interactive sends.
     *
     * Required: client_id, to_emails (comma-separated), subject, body_html
     * Optional: cc_emails, bcc_emails, template_id, attachments (file uploads)
     *
     * Returns JSON response with success/failure status.
     */
    public function silentSend(Request $request)
    {
        $request->validate([
            'client_id' => 'required|integer',
            'to_emails' => 'required|string',
        ...
        // Auto-append signature and disclaimer
        $signature = $this->getUserSignature();
        $signatureHtml = $this->buildSignatureHtml($signature);
        $disclaimerHtml = $this->buildDisclaimerHtml($signature);
        ...
        $rawBodyHtml = $request->body_html;
        if ($signatureHtml) {
            $rawBodyHtml .= '<br>' . $signatureHtml;
        }
        if ($disclaimerHtml) {
            $rawBodyHtml .= $disclaimerHtml;
        }

        // Wrap in proper HTML email document
        $bodyHtml = '<!DOCTYPE html>'
            . '<html lang="en" xmlns="http://www.w3.org/1999/xhtml">'
            . '<head>'
            . '<meta charset="UTF-8">'
        ...
        // Store email record (same audit trail as interactive send)
        $emailId = DB::table('cims_emails')->insertGetId([
            'client_id' => $request->client_id ?: null,
            'user_id' => Auth::id(),
            'from_email' => $fromEmail,
        ...
        try {
            ...
            \Log::info('CIMS Silent Email sending', [
                'email_id' => $emailId,
                'to' => $validTo,
                'cc' => $validCc,
                'bcc' => $validBcc,
            ...
            foreach ($validTo as $toEmail) {
                ...
                $sentCount++;
                \Log::info('CIMS Silent Email delivered to: ' . $toEmail . ' (send #' . $sentCount . ')');
            }
            ...
            \Log::error('CIMS Silent Email failed', ['email_id' => $emailId, 'error' => $e->getMessage()]);
