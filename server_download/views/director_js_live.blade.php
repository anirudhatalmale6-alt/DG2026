       ...
       function validateDirectorConstraints() {
        ...
        function validateDirectorForm() {
            ...
            if (!validation.isValid) {
                ...
                // Show SweetAlert with all constraint errors for visibility
                var errorHtml = '<ul style="text-align:left; margin:0; padding-left:20px;">';
                validation.errors.forEach(function(err) {
                    errorHtml += '<li style="margin-bottom:8px;">' + err.message + '</li>';
                });
                errorHtml += '</ul>';
        ...
        function displaySavedDirectors() {
            ...
            savedDirectors.forEach(director => {
                ...
                const directorShares = director.director_type_id == 1 ? parseInt(String(director.number_of_director_shares || '0').replace(/[,\s]/g, ''), 10) || 0 : 0;
                const companyShares = parseInt(String($('#number_of_shares').val() || '0').replace(/[,\s]/g, ''), 10) || 0;
                const sharePercentage = (director.director_type_id == 1 && companyShares > 0) ? ((directorShares / companyShares) * 100).toFixed(2) : '0.00';
                const number_of_director_shares = director.director_type_id == 1 ? `<dt style="width:14rem">Shares:</dt><dd>${director.number_of_director_shares}</dd><dt style="width:14rem">Shareholding %:</dt><dd>${sharePercentage}%</dd>` : '';
            ...
            // ── Totals Summary ──
            const toInt = (value) => parseInt(String(value || '').replace(/[,\s]/g, ''), 10) || 0;
            const companyTotalShares = toInt($('#number_of_shares').val());
            const activeShareholderDirectors = savedDirectors.filter(d =>
                !d.is_deleted && String(d.director_type_id) === '1' && String(d.director_status_id) === '1'
            );
            const totalAllocatedShares = activeShareholderDirectors.reduce((sum, d) => sum + toInt(d.number_of_director_shares), 0);
            const totalPercentage = companyTotalShares > 0 ? ((totalAllocatedShares / companyTotalShares) * 100) : 0;
            const remainingShares = Math.max(0, companyTotalShares - totalAllocatedShares);
            const isFullyAllocated = companyTotalShares > 0 && totalAllocatedShares === companyTotalShares;
            const isOver = totalAllocatedShares > companyTotalShares;

            const statusColor = isOver ? '#dc3545' : (isFullyAllocated ? '#28a745' : '#ffc107');
            const statusText = isOver ? 'Over-allocated' : (isFullyAllocated ? 'Fully Allocated' : 'Partially Allocated');
            const statusIcon = isOver ? 'fa-exclamation-triangle' : (isFullyAllocated ? 'fa-check-circle' : 'fa-info-circle');

            if (activeShareholderDirectors.length > 0) {
                const $totals = $(`
                    <div class="col-12 mt-3">
                        <div class="multi-card shadow-md" style="background: linear-gradient(135deg, #1a1f36 0%, #2d3561 100%); color: #fff; border-radius: 10px;">
                            <div class="card-body" style="padding: 20px 25px;">
                                <h5 style="margin-bottom: 15px; font-weight: 600; font-size: 18px;">
                                    <i class="fa fa-chart-pie me-2"></i> Shareholding Summary
                                </h5>
                                <div class="row text-center">
                                    <div class="col-md-3">
                                        <div style="font-size: 28px; font-weight: 700;">${companyTotalShares.toLocaleString()}</div>
                                        <div style="font-size: 13px; opacity: 0.7;">Company Total Shares</div>
                                    </div>
                                    <div class="col-md-3">
                                        <div style="font-size: 28px; font-weight: 700; color: ${statusColor};">${totalAllocatedShares.toLocaleString()}</div>
                                        <div style="font-size: 13px; opacity: 0.7;">Allocated Shares</div>
                                    </div>
                                    <div class="col-md-3">
                                        <div style="font-size: 28px; font-weight: 700; color: ${statusColor};">${totalPercentage.toFixed(2)}%</div>
                                        <div style="font-size: 13px; opacity: 0.7;">Total Percentage</div>
                                    </div>
                                    <div class="col-md-3">
                                        <div style="font-size: 28px; font-weight: 700;">${remainingShares.toLocaleString()}</div>
                                        <div style="font-size: 13px; opacity: 0.7;">Remaining Shares</div>
                                    </div>
                                </div>
                                <div class="text-center mt-3">
                                    <span class="badge" style="background: ${statusColor}; color: #fff; padding: 8px 18px; font-size: 14px; border-radius: 50px;">
