...
<style>
    /* Neaten Select All / Deselect All buttons in Services multiselect */
    #service_ids ~ .dropdown-menu .bs-actionsbox .btn-group {
        display: flex;
        gap: 4px;
        padding: 6px 8px;
    }
    #service_ids ~ .dropdown-menu .bs-actionsbox .btn-group .btn {
        flex: 1;
        font-size: 12px;
        padding: 4px 10px;
        border-radius: 4px;
    ...
    #service_ids ~ .dropdown-menu .bs-actionsbox .btn-group .btn:hover {
        background: #e9ecef;
        border-color: #adb5bd;
    }
</style>
    ...
    <div class="row">
        <div class="col-12">
            <div class="card smartdash-form-card">
                            ...
                            <div class="col-12">
                               ...
                               <div class="row"  id="saved-director-list">
                                ...
                                <div class="form-section-title pt-5">
                                    <i class="fa fa-toggle-on"></i> General
                                </div>

                                <div class="row mb-3">
                                    <div class="col-md-4">
                                        <label class="form-label">Status</label>
                                        <select name="client_status_id" id="client_status_id" class="default-select sd_drop_class" data-live-search="true" title="Select Status">
                                            <option value="">Select Status</option>
                                            @foreach($clientStatuses ?? [] as $status)
                                                <option value="{{ $status->id }}" {{ old('client_status_id', $client->client_status_id ?? '') == $status->id ? 'selected' : '' }}>{{ $status->status_name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-4">
                                        <label class="form-label">Category</label>
                                        <select name="client_category_id" id="client_category_id" class="default-select sd_drop_class" data-live-search="true" title="Select Category">
                                            <option value="">Select Category</option>
                                            @foreach($clientCategories ?? [] as $category)
                                                <option value="{{ $category->id }}" {{ old('client_category_id', $client->client_category_id ?? '') == $category->id ? 'selected' : '' }}>{{ $category->category_name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-4">
                                        <label class="form-label">Services</label>
                                        <select name="service_ids[]" id="service_ids" class="default-select sd_drop_class" multiple data-live-search="true" title="Select Services" data-actions-box="true" data-selected-text-format="count > 3">
                                            @php $selSvcIds = old('service_ids', $selectedServiceIds ?? []); @endphp
                                            @foreach($masterServices ?? [] as $service)
                                                <option value="{{ $service->id }}" {{ in_array($service->id, (array)$selSvcIds) ? 'selected' : '' }}>{{ $service->service_name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>

                                {{-- Active Client hidden - always defaults to 1 --}}
                ...
                <div id="bank-files-container" style="display:none;"></div>
...
// Trading name - no auto-formatting, keep as typed
...
$('#company_name').on('blur', function() {
    ...
    $.ajax({
        ...
        success: function(response) {
            // Keep the name as typed - only trim whitespace
            $input.val(name);
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        $('#client_form').on('submit', function(e) {
            ...
            newBanks.forEach((bank, index) => {
                ...
                // Handle file: rename the preserved file input to match expected name
                const $bankFile = $('#bank_file_' + bank.id);
                if ($bankFile.length) {
                    $bankFile.attr('name', `banks[${index}][confirmation_file]`);
                    $bankFile.addClass('bank-hidden-input');
                }
            });
