@php
    ...
    $cacheVersion = @file_get_contents(storage_path('cache_version.txt')) ?: '1';
@endphp
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <link href="/public/smartdash/css/smartdash-forms.css?v={{ $cacheVersion }}" rel="stylesheet">
    <link href="/assets/css/style.css?v={{ $cacheVersion }}" rel="stylesheet">
    <link href="/public/modules/cimscore/css/cims_master_system_settings.css?v={{ $cacheVersion }}" rel="stylesheet">
...
<script src="/public/modules/cimscore/js/cims_master_sweetalert.js?v={{ $cacheVersion }}"></script>
<script src="/public/smartdash/vendor/toastr/js/toastr.min.js"></script>
<script src="/public/smartdash/js/custom.min.js?v={{ $cacheVersion }}"></script>
<script src="/public/smartdash/js/dlabnav-init.js?v={{ $cacheVersion }}"></script>

@stack('scripts')

<!-- Clear Cache Overlay -->
<div class="cims-cache-overlay" id="cimsCacheOverlay">
    <div class="cims-cache-box">
        <div class="cims-cache-spinner"></div>
        <div class="cims-cache-text">Clearing Cache...</div>
    </div>
</div>
<script>
function cimsClearCache(url) {
    document.getElementById('cimsCacheOverlay').classList.add('active');
