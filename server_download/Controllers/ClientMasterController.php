...
class ClientMasterController extends Controller
{
    ...
    public function create()
    {
        ...
        $clientStatuses = DB::table('cims_client_status')->where('isactive', 1)->where('isdelete', 0)->get();
        $clientCategories = DB::table('cims_client_category')->where('isactive', 1)->where('isdelete', 0)->get();
        $masterServices = DB::table('cims_master_services')->where('isactive', 1)->where('isdelete', 0)->orderBy('service_name')->get();

        return view('cims_pm_pro::clientmaster.clientmaster_create', compact('lookups', 'clientCode', 'addresses', 'share_types', 'vat_cycles', 'client_titles', 'client_positions', 'banks', 'existingBanks', 'existingAddresses', 'accountTypes', 'address_types', 'persons', 'director_types', 'director_statuses', 'clientStatuses', 'clientCategories', 'masterServices'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Save client services (many-to-many)
            $this->syncClientServices($client->client_id, $request->input('service_ids', []));
    ...
    public function edit($id)
    {
        ...
        $clientCategories = DB::table('cims_client_category')->where('isactive', 1)->where('isdelete', 0)->get();
        $masterServices = DB::table('cims_master_services')->where('isactive', 1)->where('isdelete', 0)->orderBy('service_name')->get();
        $selectedServiceIds = DB::table('cims_client_service')->where('client_id', $client->client_id)->where('isactive', 1)->where('isdelete', 0)->pluck('service_id')->toArray();

        return view('cims_pm_pro::clientmaster.clientmaster_create', compact('client', 'lookups', 'addresses', 'share_types', 'vat_cycles', 'client_titles', 'client_positions', 'banks', 'existingBanks', 'existingAddresses', 'address_types', 'persons', 'director_types', 'director_statuses', 'existingDirectors', 'clientStatuses', 'clientCategories', 'masterServices', 'selectedServiceIds'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            // Sync client services (many-to-many)
            $this->syncClientServices($client->client_id, $request->input('service_ids', []));
    ...
    /**
     * Sync client services (many-to-many via pivot table)
     */
    private function syncClientServices($clientId, array $serviceIds)
    {
        // Remove all existing active services for this client
        DB::table('cims_client_service')
            ->where('client_id', $clientId)
            ->delete();
        ...
        if (!empty($rows)) {
            DB::table('cims_client_service')->insert($rows);
        }
    }
    ...
    {
        return view('cims_pm_pro::clientmaster.system_settings');
    }
    ...
    /**
     * Client Capture Form - page with blank download + completed form selector
     */
    public function captureForm()
    {
        ...
        return view('cims_pm_pro::clientmaster.capture_form', compact('clients', 'selectedClientId'));
    }

    /**
     * Blank Capture Form PDF
     */
    public function captureFormBlankPdf()
    {
        $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView('cims_pm_pro::clientmaster.capture_form_blank_pdf');
        $pdf->setPaper('a4', 'portrait');

        $filename = 'CIM Solutions - Blank Capture Form - ' . now()->format('d M Y') . '.pdf';

        return $pdf->stream($filename);
    }

    /**
     * Completed Capture Form PDF - pre-filled with client data
     */
    public function captureFormCompletedPdf(Request $request)
    {
        $clientId = $request->get('client_id');
        if (!$clientId) {
            return back()->with('error', 'No client selected.');
        ...
        // Get all addresses
        $addresses = \DB::table('client_master_addresses')
            ->where('client_id', $clientId)
            ->get();

        // Get all bank accounts
        $banks = \DB::table('client_master_banks')
            ->where('client_id', $clientId)
            ->where('is_active', 1)
            ->get();

        // Get services as a string
        $services = '';

        // Base64 encode header image for DomPDF compatibility
        $assetsBase = base_path('../storage/assets');
        $headerImgPath = $assetsBase . '/info_sheet_header.jpg';
        $headerImgBase64 = '';
        if (file_exists($headerImgPath)) {
        ...
        $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView('cims_pm_pro::clientmaster.capture_form_completed_pdf', compact(
            'client', 'directors', 'addresses', 'banks', 'services', 'headerImgBase64'
        ));

        $pdf->setPaper('a4', 'portrait');

        $filename = $client->client_code . ' - Capture Form - ' . now()->format('d M Y') . '.pdf';
