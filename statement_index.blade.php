...
<style>
    ...
    /* --- Client dropdown in breadcrumb_master --- */
    .breadcrumb_master .bm_controls select.form-control {
        min-height: 48px;
        border-radius: 8px;
        font-size: 14px;
        font-family: 'Poppins', sans-serif;
    ...
    .breadcrumb_master .bm_controls select.form-control:focus {
        border-color: #17A2B8;
        box-shadow: 0 0 0 0.2rem rgba(23,162,184,0.25);
        outline: none;
    }
    /* Override date picker min-width to fit inside col-2 grid */
    .breadcrumb_master .bm_controls .date_master_picker {
        width: 100%;
    }
    .breadcrumb_master .bm_controls .date_master_picker input {
        min-width: auto;
        width: 100%;
        font-size: 13px;
    }
    ...
    <div class="breadcrumb_master">
        ...
        <div class="bm_header">
            <div class="bm_title_area">
                ...
                <div>
                    <div class="bm_title">Customer Statement of Account</div>
                    <div class="bm_subtitle">Client Invoices &amp; Payments Statement</div>
                </div>
            </div>
            <div class="bm_badge">STATEMENT</div>
        </div>
        <!-- Row 2: Controls -->
        <div class="bm_controls">
            <div class="row align-items-end w-100" style="margin:0;">
                <div class="col-md-6" style="padding-left:0;">
                    <label class="date_master_picker_label">Client</label>
                ...
                <div class="col-md-2">
                    <label class="date_master_picker_label">From Date</label>
                    <div class="date_master_picker">
                        <input type="text" id="fromDate" placeholder="Select date..." readonly>
                        <i class="fa-regular fa-calendar-days dm_icon"></i>
                    </div>
                </div>
                <div class="col-md-2">
                    <label class="date_master_picker_label">To Date</label>
                    <div class="date_master_picker">
                        <input type="text" id="toDate" placeholder="Select date..." readonly>
                        <i class="fa-regular fa-calendar-days dm_icon"></i>
                    </div>
                </div>
                <div class="col-md-2" style="padding-right:0;">
                    <label class="date_master_picker_label">&nbsp;</label>
                    <button class="button_master_generate w-100" id="btnGenerate"><i class="fa fa-sync-alt"></i> Generate</button>
                </div>
            </div>
        </div>
        <!-- Row 3: Quick Select Pills -->
        <div class="bm_pills">
            <label class="bm_pills_label">Quick Select:</label>
            <div class="bm_pills_content">
                <div class="bm_pills_row">
                    <button type="button" class="bm_pill" data-period="today">Today</button>
                    <button type="button" class="bm_pill" data-period="this_week">This Week</button>
                    <button type="button" class="bm_pill" data-period="this_fortnight">This Fortnight</button>
                    <button type="button" class="bm_pill" data-period="this_month">This Month</button>
                    <button type="button" class="bm_pill" data-period="last_month">Last Month</button>
                    <button type="button" class="bm_pill" data-period="this_quarter">This Quarter</button>
                    <button type="button" class="bm_pill" data-period="last_quarter">Last Quarter</button>
                    <button type="button" class="bm_pill" data-period="last_3_months">Last 3 Months</button>
                    <button type="button" class="bm_pill" data-period="last_6_months">Last 6 Months</button>
                </div>
                <div class="bm_pills_row">
                    <button type="button" class="bm_pill" data-period="this_year">This Calendar Year</button>
                    <button type="button" class="bm_pill" data-period="last_year">Last Calendar Year</button>
                    <button type="button" class="bm_pill" data-period="this_fin_year">This Financial Year</button>
                    <button type="button" class="bm_pill" data-period="last_fin_year">Last Financial Year</button>
                    <button type="button" class="bm_pill" data-period="all">All Transactions</button>
                </div>
            </div>
        </div>
        ...
        <div class="bm_actions">
            ...
            <button class="button_master_pdf no-print" id="btnExportPDF" style="display:none;"><i class="fa fa-file-pdf"></i> PDF</button>
...
(function(){
    ...
    var fpFrom = flatpickr('#fromDate', {
        dateFormat: 'j M Y',
        defaultDate: '{{ \Carbon\Carbon::parse($defaultFrom)->format("Y-m-d") }}',
        allowInput: false
    });
    var fpTo = flatpickr('#toDate', {
        dateFormat: 'j M Y',
        defaultDate: '{{ \Carbon\Carbon::parse($defaultTo)->format("Y-m-d") }}',
        allowInput: false
    });
    ...
    // ========== QUICK SELECT PILLS ==========
    $('.bm_pill').on('click', function(){
        var period = $(this).data('period');
        var now = new Date();
        var from, to;

        // SA Financial year: 1 March – 28/29 February
        var finYearStart, finYearEnd;
        ...
        switch(period) {
            ...
            case 'last_year':
                ...
                break;
            case 'this_fin_year':
                // SA Fin Year: 1 Mar current year → 28 Feb next year (if we're past March)
                if (now.getMonth() >= 2) { // March (0-indexed) or later
                    finYearStart = new Date(now.getFullYear(), 2, 1);
                    finYearEnd = new Date(now.getFullYear() + 1, 1, 28);
                } else {
                    finYearStart = new Date(now.getFullYear() - 1, 2, 1);
                    finYearEnd = new Date(now.getFullYear(), 1, 28);
                }
                from = finYearStart;
                to = finYearEnd;
                break;
            case 'last_fin_year':
                if (now.getMonth() >= 2) {
                    ...
                    finYearStart = new Date(now.getFullYear() - 2, 2, 1);
                    finYearEnd = new Date(now.getFullYear() - 1, 1, 28);
                }
                from = finYearStart;
                to = finYearEnd;
    ...
    $('#btnGenerate').on('click', function(){
        ...
        $container.hide();
        $('#btnExcel, #btnExportPDF, #btnEmail, #btnPrint').hide();
        $spinner.show();
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $container.show();
                $('#btnExcel, #btnExportPDF, #btnEmail, #btnPrint').show();
            },
            error: function(xhr) {
                $spinner.hide();
    ...
    function renderStatement(data) {
        ...
        if (transactions.length === 0) {
            ...
            for (var i = 0; i < transactions.length; i++) {
                ...
                if (txn.type === 'Invoice' && txn.id) {
                    html += '  <td style="' + refStyle + '"><a href="/invoices/' + txn.id + '" target="_blank" style="color:#17A2B8;text-decoration:none;font-weight:600;" onmouseover="this.style.textDecoration=\'underline\'" onmouseout="this.style.textDecoration=\'none\'">' + esc(txn.reference) + '</a></td>';
                } else if (txn.type === 'Payment' && txn.id) {
                    html += '  <td style="' + refStyle + '"><a href="/payments/v/' + txn.id + '" target="_blank" style="color:#28a745;text-decoration:none;font-weight:600;" onmouseover="this.style.textDecoration=\'underline\'" onmouseout="this.style.textDecoration=\'none\'">' + esc(txn.reference) + '</a></td>';
                } else {
    ...
    $('#btnExcel').on('click', function(){
        ...
        var client = lastData.client;
        var transactions = lastData.transactions;
        var currency = lastData.settings.settings_system_currency_symbol || 'R ';
        var company = lastData.settings.settings_company_name || '';

        var headers = ['Date', 'Type', 'Reference', 'Description', 'Debit', 'Credit', 'Balance'];
        var headerAlign = ['left', 'left', 'left', 'left', 'right', 'right', 'right'];
        var colWidths = [16, 12, 18, 36, 16, 16, 18];
        var rows = [];

        // Opening balance row
        rows.push({
            cells: [formatDisplayDate(lastData.from_date), '', '', 'Opening Balance', '', '', parseFloat(lastData.opening_balance) || 0],
            styles: { 3: { font: 'bold' }, 6: { font: 'bold' } }
        });
        ...
        for (var i = 0; i < transactions.length; i++) {
            ...
            rows.push({
                cells: [
                    ...
                    txn.reference,
                    txn.description,
                    txn.debit > 0 ? parseFloat(txn.debit) : '',
                    txn.credit > 0 ? parseFloat(txn.credit) : '',
                    parseFloat(txn.balance) || 0
                ],
                styles: txn.type === 'Invoice' ? { 1: { font: 'teal' } } : { 1: { font: 'boldText' } }
            });
        }

        var clientName = client.client_company_name || '';
        var fromISO = getFromISO();
        var toISO = getToISO();

        cimsExcelExport({
            title: 'STATEMENT OF ACCOUNT',
            subtitle: clientName + '  |  ' + formatDisplayDate(fromISO) + ' to ' + formatDisplayDate(toISO),
            company: company,
            date: new Date().toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }),
            filename: 'Statement_' + (clientName.replace(/[^a-zA-Z0-9]/g, '_')) + '_' + fromISO + '_to_' + toISO,
            headers: headers,
            ...
            totals: {
                ...
                values: [
                    parseFloat(lastData.total_debits) || 0,
                    parseFloat(lastData.total_credits) || 0,
                    parseFloat(lastData.closing_balance) || 0
                ]
            }
        });
    });
